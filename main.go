...
func main() {
	...
	spdr := &Spider{
		AccessPoints: make([]*spider.AccessPoint, 0),
		Adapters: make([]*spider.Adapter, 0),
		AdHocs: make([]*spider.AdHoc, 0),
		BasicServiceSets: make([]*spider.BasicServiceSet, 0),
		Devices: make([]*spider.Device, 0),
		KnownNetworks: make([]*spider.KnownNetwork, 0),
		Networks: make([]*spider.Network, 0),
	...
	for _, on := range ons {
		if on.Network.GetName() == "ShadowGate" {
			mynet = on.Network
			break
		}
	}
	...
	if mynet == nil {
		fmt.Printf("Network ShadowGate not found\n")
		os.Exit(129)
	}
